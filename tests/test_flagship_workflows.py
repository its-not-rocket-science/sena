...
def test_workflow_a_jira_approval_gating_failure_mode_blocks_high_risk_without_cab() -> (
    None
):
    cfg = load_jira_mapping_config(
        "examples/design_partner_reference/integration/jira_mapping.yaml"
...
def test_workflow_a_jira_failure_mode_missing_actor_is_deterministic() -> None:
    ...
    with pytest.raises(JiraIntegrationError, match="missing actor identity"):
        connector.handle_event(
            {
                "headers": {
                    "x-atlassian-webhook-identifier": "flagship-jira-missing-actor"
                },
                "payload": payload,
                "raw_body": json.dumps(payload).encode("utf-8"),
            }
