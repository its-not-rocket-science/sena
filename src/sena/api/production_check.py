...
class CheckResult:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"name": self.name, "status": self.status, "fatal": self.fatal, "details": self.details}
...
def _writable_dir(path: Path) -> bool:
    if not path.exists() or not path.is_dir():
        return False
    try:
        with tempfile.NamedTemporaryFile(prefix="sena-write-check-", dir=path, delete=True):
            return True
    except OSError:
        return False
...
def _validate_env_coherence(settings: ApiSettings) -> list[str]:
    ...
    if settings.policy_store_backend == "filesystem":
        ...
        if not policy_dir.exists() or not policy_dir.is_dir():
            errors.append(f"SENA_POLICY_DIR must point to an existing directory: {settings.policy_dir}")
    if settings.policy_store_backend == "sqlite":
        if not settings.policy_store_sqlite_path:
            errors.append("SENA_POLICY_STORE_SQLITE_PATH is required when SENA_POLICY_STORE_BACKEND=sqlite")
        else:
            sqlite_parent = Path(settings.policy_store_sqlite_path).expanduser().resolve().parent
    ...
    if settings.enable_api_key_auth and not settings.api_key and not settings.api_keys:
        errors.append("SENA_API_KEY_ENABLED=true requires SENA_API_KEY or SENA_API_KEYS to be set")
    ...
    for config_path, env_name in (
        ...
        if config_path:
            ...
            if not path.exists() or not path.is_file():
                errors.append(f"{env_name} must point to an existing file: {config_path}")
    ...
    if settings.runtime_mode == "production":
        ...
        if not settings.bundle_signature_strict:
            errors.append("SENA_RUNTIME_MODE=production requires SENA_BUNDLE_SIGNATURE_STRICT=true")
        if not settings.bundle_signature_keyring_dir:
            errors.append("SENA_RUNTIME_MODE=production requires SENA_BUNDLE_SIGNATURE_KEYRING_DIR")
    ...
    if settings.audit_verify_on_startup_strict and not settings.audit_sink_jsonl:
        errors.append("SENA_AUDIT_VERIFY_ON_STARTUP_STRICT=true requires SENA_AUDIT_SINK_JSONL")
    return errors
...
def _policy_backend_ready(settings: ApiSettings) -> list[str]:
    ...
    if settings.policy_store_backend == "sqlite":
        if not settings.policy_store_sqlite_path:
            return ["SENA_POLICY_STORE_SQLITE_PATH is required when SENA_POLICY_STORE_BACKEND=sqlite"]
        repo = SQLitePolicyBundleRepository(settings.policy_store_sqlite_path)
        repo.initialize()
        active = repo.get_active_bundle(settings.bundle_name)
        if active is None:
            errors.append(f"No active bundle found for '{settings.bundle_name}' in sqlite store")
        elif not active.rules:
            errors.append(f"Active bundle '{settings.bundle_name}' in sqlite store has no rules")
        return errors
    try:
...
def _signature_ready(settings: ApiSettings) -> list[str]:
    manifest_path = Path(settings.policy_dir) / settings.bundle_release_manifest_filename
    ...
    result = verify_release_manifest(
        ...
        keyring_dir=Path(settings.bundle_signature_keyring_dir) if settings.bundle_signature_keyring_dir else None,
...
def run_production_readiness_check(settings: ApiSettings) -> dict[str, Any]:
    ...
    def add(name: str, *, fatal: bool, details: list[str]) -> None:
        checks.append(CheckResult(name=name, status="pass" if not details else "fail", fatal=fatal and bool(details), details=details))

    add("environment variable coherence", fatal=True, details=_validate_env_coherence(settings))

    auth_details: list[str] = []
    if settings.enable_api_key_auth and not (settings.api_key or settings.api_keys):
        auth_details.append("API key auth is enabled but no API key material is configured")
    if settings.runtime_mode == "production" and not settings.enable_api_key_auth:
    ...
    if settings.request_timeout_seconds > 300:
        request_details.append("SENA_REQUEST_TIMEOUT_SECONDS is above 300s; this is likely unsafe for production")
    add("request limits and timeout sanity", fatal=True, details=request_details)

    add("policy store backend readiness", fatal=True, details=_policy_backend_ready(settings))
    add("signature verification configuration", fatal=True, details=_signature_ready(settings))
    ...
    for label, config_path, loader in (
        ...
        ("servicenow", settings.servicenow_mapping_config_path, load_servicenow_mapping_config),
    ):
        if not config_path:
            continue
        try:
    ...
    add("integration mapping existence and schema validity", fatal=True, details=integration_details)
    ...
    if settings.audit_sink_jsonl:
        ...
        if not _writable_dir(parent):
            audit_details.append(f"audit sink parent directory is not writable: {parent}")
    ...
    if settings.policy_store_backend == "sqlite" and settings.policy_store_sqlite_path:
        ...
        if not _writable_dir(sqlite_parent):
            writable_details.append(f"sqlite parent directory is not writable: {sqlite_parent}")
    if settings.bundle_signature_keyring_dir:
        keyring = Path(settings.bundle_signature_keyring_dir)
        if not keyring.exists() or not keyring.is_dir() or not os.access(keyring, os.R_OK):
    ...
    fatal_failures = [check.name for check in checks if check.status == "fail" and check.fatal]
    return {
